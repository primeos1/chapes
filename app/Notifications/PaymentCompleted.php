...
class PaymentCompleted extends Notification {
    ...
    public function __construct($paymentRequest) {
        ...
        $this->template       = EmailSMSTemplate::where('slug', 'PAYMENT_COMPLETED')->first();

        $this->replace['name']     = $this->paymentRequest->sender->name;
        $this->replace['email']    = $this->paymentRequest->sender->email;
        $this->replace['phone']    = $this->paymentRequest->sender->phone;
        $this->replace['paidBy']   = $this->paymentRequest->receiver->name;
