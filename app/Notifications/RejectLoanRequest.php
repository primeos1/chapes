...
class RejectLoanRequest extends Notification {
    ...
    public function __construct($loan) {
        ...
        $this->loan     = $loan;
        $this->template = EmailSMSTemplate::where('slug', 'LOAN_REQUEST_REJECTED')->first();

        $this->replace['name']     = $this->loan->borrower->name;
        $this->replace['email']    = $this->loan->borrower->email;
        $this->replace['phone']    = $this->loan->borrower->phone;
